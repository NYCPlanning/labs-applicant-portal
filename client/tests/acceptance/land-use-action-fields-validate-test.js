...
module('Acceptance | land use action fields validate', function (hooks) {
  ...
  test('validation message appears for count field when user types incorrect value', async function (assert) {
    ...
    assert
      .dom('[data-test-validation-message="dcpPfzoningauthorization"]')
      .doesNotExist();
    ...
    assert
      .dom('[data-test-validation-message="dcpPfzoningauthorization"]')
      .hasText('Number of actions must be greater than 0');
  ...
  test('validation message appears for extra questions when user types incorrect value', async function (assert) {
    ...
    assert
      .dom('[data-test-validation-message="dcpZoningauthorizationpursuantto"]')
      .doesNotExist();
    assert
      .dom('[data-test-validation-message="dcpZoningauthorizationtomodify"]')
      .doesNotExist();
    ...
    await fillIn(
      '[data-test-input="dcpZoningauthorizationpursuantto"]',
      maximum(250, 'a'),
    );
    assert
      .dom('[data-test-validation-message="dcpZoningauthorizationpursuantto"]')
    ...
    await fillIn(
      '[data-test-input="dcpZoningauthorizationtomodify"]',
      maximum(250, 'a'),
    );
    assert
      .dom('[data-test-validation-message="dcpZoningauthorizationtomodify"]')
    ...
    assert
      .dom('[data-test-validation-message="dcpPreviousulurpnumbers2"]')
      .hasText('This field is required');
    await fillIn(
      '[data-test-input="dcpPreviousulurpnumbers2"]',
      maximum(100, '5'),
    );
    assert
      .dom('[data-test-validation-message="dcpPreviousulurpnumbers2"]')
