...
module('Acceptance | error message appears when save fails', function (hooks) {
  ...
  test('error message appears when error occurs on save for PAS Form', async function (assert) {
    ...
    this.server.patch(
      '/pas-forms/:id',
      { errors: [{ detail: 'server problem with pasForm' }] },
      500,
    ); // force mirage to error
    ...
    assert
      .dom('[data-test-error-key="detail"]')
      .includesText('server problem with pasForm');
  ...
  test('error message appears when error occurs on save for RWCDS Form', async function (assert) {
    ...
    this.server.patch(
      '/rwcds-forms/:id',
      { errors: [{ detail: 'server problem with rwcdsForm' }] },
      500,
    ); // force mirage to error
    ...
    assert
      .dom('[data-test-error-key="detail"]')
      .includesText('server problem with rwcdsForm');

    // check that model was not saved
    assert.equal(
      this.server.db.rwcdsForms[0].dcpProjectsitedescription,
      undefined,
    );
  });
  ...
  test('error message appears when error occurs on file upload', async function (assert) {
    ...
    this.server.patch(
      '/packages/:id',
      { errors: [{ detail: 'server problem with package' }] },
      500,
    ); // force mirage to error
    ...
    assert
      .dom('[data-test-error-key="detail"]')
      .includesText('server problem with package');
