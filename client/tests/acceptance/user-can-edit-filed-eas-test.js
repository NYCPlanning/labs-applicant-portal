...
module('Acceptance | user can edit Filed EAS Packages', function (hooks) {
  ...
  test('User sees Project under Awaiting Submission if it has an active Filed EAS', async function (assert) {
    ...
    this.server.create('project', {
      packages: [this.server.create('package', 'done', 'filedEas')],
    });
  ...
  test('User sees Filed EAS Packages section on Project page when one exists', async function (assert) {
    ...
    assert
      .dom(
        '[data-test-package-section="Environmental Assessment Statement (EAS)"]',
      )
      .exists();
  ...
  test('User can click into Filed EAS and see Package info, Package notes, and Attachments', async function (assert) {
    this.server.create('project', {
      packages: [this.server.create('package', 'toDo', 'filedEas')],
    });
  ...
  test('User can submit Filed EAS and see Package info and Attached Documents section', async function (assert) {
    ...
    await click(
      '[data-test-radio="dcpIsthesoleaapplicantagovtagency"][data-test-radio-option="Yes"]',
    );
  ...
  test('User sees CEQR on the Filed EAS Show page', async function (assert) {
    this.server.create('project', {
      packages: [this.server.create('package', 'toDo', 'scopeOfWorkDraft')],
    });
  ...
  test('User can click through Ceqr Invoice Questionnaire section on Filed Eas', async function (assert) {
    ...
    await click(
      '[data-test-radio="dcpIsthesoleaapplicantagovtagency"][data-test-radio-option="No"]',
    );
    await click(
      '[data-test-radio="dcpProjectspolelyconsistactionsnotmeasurable"][data-test-radio-option="No"]',
    );
    await selectChoose(
      '[data-test-dcpsquarefeet-picker]',
      'less than 10,000 square feet',
    );
    await click(
      '[data-test-radio="dcpProjectmodificationtoapreviousapproval"][data-test-radio-option="No"]',
    );
    await click(
      '[data-test-radio="dcpRespectivedecrequired"][data-test-radio-option="Yes"]',
    );
