...
@UseInterceptors(
  new JsonApiSerializeInterceptor('project-applicants', {
...
export class ProjectApplicantController {
  ...
  async create(@Body() body) {
      ...
      throw new HttpException(
        {
          ...
          detail:
            'Cannot add project applicant because missing email or project id',
        },
    ...
    if (records.length > 0) {
      ...
      if (
        records[0].statuscode === INACTIVE_STATUSCODE &&
        records[0].statecode === INACTIVE_STATECODE
      ) {
    ...
    const newProjectApplicant = await this.crmService.create(
