...
export class PackagesService {
  ...
  async getPackage(packageId) {
    ...
    // Get package
    try {
      console.log('CALL CRM dcp_packages');
      const {
        ...
        &$expand=dcp_project($select=${PROJECT_ATTRS.join(
          ',',
        )}),dcp_package_dcp_ceqrinvoicequestionnaire_Package,dcp_package_SharePointDocumentLocations
      `,
      );
      ...
      if (documentLocations && documentLocations.length > 0) {
        ...
        console.log('CALL getPackageSharepointDocuments');
      ...
      if (typeof projectArtifacts === 'undefined') {
        console.log('projectArtifacts IS UNDEFINED');
      } else {
        console.log('projectArtifacts:', projectArtifacts);
      }

      let firstArtifactWithDocuments = {};

      if (typeof projectArtifacts.length === 'undefined') {
        console.log('projectArtifacts.length IS UNDEFINED');
      } else {
        console.log('projectArtifacts.length:', projectArtifacts.length);
      }

      if (projectArtifacts.length < 1) {
        // TODO: Consider reducing this side effecct in this GET endpoint
        console.log('PRE createEquityReport');
        ...
        console.log('projectArtifacts.length < 1 ELSE BLOCK');
        firstArtifactWithDocuments = projectArtifacts[0];
        console.log('firstArtifactWithDocuments', firstArtifactWithDocuments);
      }

      try {
        console.log('PRE artifactWithDocuments');
        firstArtifactWithDocuments =
        ...
        throw new HttpException(
          {
            ...
            detail: `An unknown error occured while loading package ${packageId}: ${JSON.stringify(
              e,
              null,
              2,
            )}`,
